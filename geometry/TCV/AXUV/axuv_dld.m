function axuv=axuv_dld(shotno,tbegin,tend,period,array)
...
% 2015.10: cleaning. This will be the official routine since crpp250
% doesn't exist anymore.
if ~exist('shotno','var'); error('Shot number is mandatory');end;
if ~exist('tbegin','var');tbegin=[];end; if isempty(tbegin), tbegin=0.4;end;
if ~exist('tend','var');tend=[];end; if isempty(tend), tend=1.8;end;
if ~exist('period','var');period=[];end; if isempty(period), period=5e-6;end;
if ~exist('array','var');array=[];end; if isempty(array), array='PLASMA';end;
clear axuv
