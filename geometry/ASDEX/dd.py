...
#libddww = ct.cdll.LoadLibrary('/afs/ipp-garching.mpg.de/aug/ads/lib64/amd64_linux26/libddww8.so.8.1.20160215')
...
def main():
    ...
    dd = shotfile()
    if not dd.Open('SXA',37203):
        raise Exception('SXA shotfile is not avalible')

    #trick to avoid loading the whole time base!
    tlen = dd.GetInfo('Time').tlen
    tbeg = (dd.GetTimebase('Time',1,1))
    tend = (dd.GetTimebase('Time',tlen,tlen))
    
    print(tbeg,tend,tlen)
