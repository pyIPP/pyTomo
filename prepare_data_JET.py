...
class JET(Tokamak):
	...
	elif  input_diagn in ['neutrony']:
	    coord = [202.2,392.2,-140,190]
	    #path = 'geometry_JET_neutrons'

	path = 'geometry_JET/'+path_tmp
	...
	if not only_prepare:
        ...
        #print shape(self.tvec), shape(self.dets)
        #print  shape(self.get_data())
        #exit()
    ...
    def SXR(self, SOURCE):
        ...
        :var list calb: Effeciency calibration for each sensor, it is list of integers or list of vectors with length == tsteps
        ...
        self.calb = [1.1, 1, 0.78]  # relative calibration, used in prepare_data()
		...
		self.wrong_dets_damaged = array([33,34, 35])-1
	    ...
	    config.wrong_dets_pref =  r_[config.wrong_dets_pref, self.dets_index[2]]   # detektor T0
	...
	#cam_x , cam_y = meshgrid(linspace(-cam_size[0]/2,cam_size[0]/2,self.cam_zoom),  linspace(-cam_size[1]/2,cam_size[1]/2, self.cam_zoom ) )

	###print shape(cam_x), shape(cam_y)
	#cam_x = cam_x.ravel().repeat(nl)
	#cam_y = cam_y.ravel().repeat(nl)
	#cam_x = cam_size[0] * (random.rand(nl* self.cam_zoom**2)-0.5)
	#cam_y = cam_size[1] * (random.rand(nl* self.cam_zoom**2)-0.5)
	#cam_z = cam_size[0] * (random.rand(nl* self.cam_zoom**2)-0.5)


	#print cam_x, cam_y

	#print shape(c), shape(d), nl, self.cam_zoom**2, res
	...
	try:
		...
		for j in trange(len(tsurf_new),desc='Loading magnetic data: '):
			...
			pmds.mdsvalue('flusur( ' +str(psisur[i])+ ',' +str(N_points) +',_xp,_yp,_br,_bz,1) ')  # retrieve data
