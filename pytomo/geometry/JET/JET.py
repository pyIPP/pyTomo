...
class JET(Tokamak):
    ...
    def __init__(self, input_diagn, input_parameters,  only_prepare = False  ):
        ...
        self.zoom_coord = self.divertor_coord
    ...
    def mag_equilibrium(self,*data, **kdict):
        rho,magx, magy = self.Tokamak.mag_equilibrium(self, *data, **kdict)
        if magx.ndim == 3:
            separatrix_x, separatrix_y = magx[:,-1,:].mean(-1), magy[:,-1,:].mean(-1)
        else:
            separatrix_x, separatrix_y = magx[:,-1], magy[:,-1]

        savetxt(self.geometry_path+'/border.txt',vstack((separatrix_x,separatrix_y)).T,fmt='%.4e')
        return magx, magy
    ...
    def SXR(self, SOURCE):
        ...
        calb_all = r_[calbV,calbS4,calbT ]
        ...
        if self.shot < self._new_calb_shot:
            ...
            self.wrong_dets_damaged = array([44,59,61])-1        # problems ???

    #			#if self.shot > 69541 & self.shot < 69943:
    #				#self.wrong_dets_damaged = array([12,44,53,55, 57, 78,81,83,85,87])-1        # problems ???
        else:
            #self.wrong_dets_damaged = array([33,34, 35,65,43])-1
            self.wrong_dets_damaged = array([43,])-1
        ...
        config.wrong_dets_pref =  unique(r_[config.wrong_dets_pref, self.dets_index[2]])   # detektor T0
        ...
        self.detectors_dict['V']=None
        self.detectors_dict['S4']=None
        self.detectors_dict['T']=None
    ...
    def Bolo(self):
        ...
        ind_ok = (tvec > 45) & (tvec < 70)  #   & (d >= 0)

        data -=  median(data[ tvec < 44,:],0)   # remove systematic bias
    ...
    def load_mag_equilibrium(self):
        ...
        try:
            ...
            d = load('geometry/JET/equilibrium/MagField_%d.npz'%self.shot)
            ...
            savez('geometry/JET/equilibrium/MagField_%d.npz'%self.shot, tsurf=self.tsurf, magx = self.magx, magy = self.magy)
                #d = load('geometry/JET/equilibrium/MagField_%d.npz'%self.shot)
