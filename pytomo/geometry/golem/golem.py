...
class Golem(Tokamak):
    def __init__(self, input_diagn, input_parameters,  only_prepare = False, Color = 0  ):
        ...
        self.min_tvec = 0
        self.max_tvec = 0.040
        self.allow_negativ = False

        path = path +input_diagn
        ...
        #self.load_mag_field()
        self.mag_field([], preferCache = False, dryRun = True)
        
        self.mag_axis =  {'tvec':[0,1], 'Rmag':[0.4,0.4], 'Zmag':[0.085,0.085 ]}
    ...
    def Bolometry(self):
        ...
        self.save_data( self.data, self.tvec)
    ...
    def Camera(self, color):
        ...
        for CAMERA in [1,2]:
            ...
            try:
                data = imread(ds.open( "http://golem.fjfi.cvut.cz/operation/shots/"\
                    +str(self.shot)+"/diagnostics/Radiation/0211FastCamera.ON/"\
                        +str(CAMERA)+"/CorrectedRGB.png"))
                NUM_CAM +=1
                if CAMERA == 1:
    ...
    def load_mag_field(self):
        ...
        try:
            ...
            magx, magy = self.convert_mag_field(magnet_field)
    ...
    def convert_mag_field(self,magnet_field, periodical = True):
        """
        Convert magnetic field from field separated by NaN to (M)x(N+1) field + (boundary), it is used by some tokamak classes. Interpolate field to have the same points in all surfaces

        Magnet_field must end with NaN and must have no NaN at the first line

        :param array mag_field: Converted magnetic field
        :var int nsurf: Number of surfaces in final field
        :rtype: array
        """

        from scipy.interpolate import splprep, splev

        nsurf = len(magnet_field[isnan(magnet_field[:,0])])
        k=0; i = 0 ; size_mag = 0
        while k < len(magnet_field):           #determine maximum number of magnetic lines (size_mag) before allocate
            i = 0
            while k < len(magnet_field) and not isnan(magnet_field[k,0]) :
                i += 1; k += 1
            size_mag = max((size_mag, i))
            k += 1

        magx =  zeros((size_mag, nsurf+1))
        magy =  zeros((size_mag, nsurf+1))
        k = 0; j = 0
        while k < len(magnet_field):
            i=0;
            while k < len(magnet_field) and  not isnan(magnet_field[k,0]) :
                magx[i,j] = magnet_field[k,0]
                magy[i,j] = magnet_field[k,1]
                i += 1; k += 1
            j += 1;k += 1
        ...
        for i in arange(nsurf):
            ...
            per = periodical
            if not periodical:
                per =  mx[0] == mx[-1] and my[0] == my[-1]  # periodical
            try:
                tckp,u = splprep([mx, my],s=0,k=3, per=int(per))
            except Exception as e:
                print("Magnetic field failed", str(e))
                tckp,u = splprep([mx[::2], my[::2]],s=0,k=3, per=int(per))
            magx[:,i],magy[:,i] = splev(linspace(0,1,size_mag),tckp)
