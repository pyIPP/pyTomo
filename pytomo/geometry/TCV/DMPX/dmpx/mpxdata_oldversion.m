...
% Modified by J. Kamleitner, CRPP, EPFL: 21.4.2011  parallelized chord load section to improve performance
% Modified by J. Kamleitner, CRPP, EPFL: 16.5.2011  changed to TCV1 for old shots to improve performance
...
%
mpxpath='/home/curchod/mpx_newpackage/';
def.action = 's';
def.time='**';
def.rtime=[0 10];
def.det='both';
...
%
if exist('chords') % If 'chords' is specified, it overrules 'detec'
	if exist('detec')
		old_detec = detec;
	end
	...
	if exist('old_detec')&~strcmp(old_detec,detec)
...
if shot<26609
	if strcmp(detec,'both'), 
		if shot<26555
			disp('Load data for top detector only: Duplex MPX not installed for shot < #26555.')
		else
			disp('Load data for top detector only: No bottom detector HV source for shot < #26609.')
		end
		detec='top';
...
%
% Opening MDS connection
%
disp(['Opening MDS connection to shot ' num2str(shot) '.']);
mdsopen(shot);
...
%
disp('Load the MPX calibrated signal')
if exist('freq')~=1
	freq=def.freq;
end
if exist('time')==1, 
...
if (shot<34988) % After TCV BIG OPENING 2007-2008, fast data are in standard nodes
	if freq~=0
		...
		if (isnumeric(test_fast) && test_fast>0),
			if freq==2,
...
if strcmp(detec,'top')|strcmp(detec,'both'),
	...
	if load_opt
        ...
        % selection of method (select serial method in any case)
        if(1 || length(chords)<32) %32
            usematmpi=0;
            useparfor=0;
        else
            usematmpi=1;
        ...
        % begin of parallel part using MatlabMPI
        if(usematmpi)
            fprintf('- Using MatMPI for parallel connections.\n');
            chords1=[];
            chords2=[];
            for ii=1:32,
                if any(find(II-ii==0)-chords==0)
                    chords1=[chords1;ii];
                end
                if any(find(II-ii-32==0)-chords==0)
                    chords2=[chords2;ii+32];
                end	
            end
            ichan=[chords1;chords2];
        
            t_parallel=tic;
            pwd_act_mpx=pwd;

            reset(RandStream.getDefaultStream,sum(100*clock));
            tmpdir=sprintf('mpxdata_%s_%d',getenv('USER'),shot*randi(1000));
            cd('/tmp');
            mkdir(tmpdir);
            cd(tmpdir);
            ...
            n=min(4,length(ichan));
            eval( MPI_Run('mpxdata_readdata_mpiscript', n) );

            cd('/tmp');
            rmdir(tmpdir,'s');

            cd(pwd_act_mpx);
            fprintf('- %d parallel data loading for DMPX took %g s.\n',length(ichan),toc(t_parallel));

        % begin of parallel part using parfor (should not be used, it is not clear if this part works properly)
        elseif(useparfor)
            iitake1=zeros(1,8);
            tmpdat1=cell(1,8);
            iitake2=zeros(1,8);
            ...
            tmpdat8=cell(1,8);
            mdsclose();
            clustertmp=findResource();
            matlabpool(min(8,clustertmp.ClusterSize))
            parfor iipar=1:8
                mdsopen(shot);
                if any(find(II-iipar==0)-chords==0)
                    iitake1(iipar)=1;
                    tmpdat1{iipar}=mdsdata([DTNE1 ch(iipar,:) echant]);
                end
                if any(find(II-iipar-8==0)-chords==0)
                    iitake2(iipar)=1;
                    tmpdat2{iipar}=mdsdata([DTNE1 ch(iipar,:) echant]);
                end
                if any(find(II-iipar-16==0)-chords==0)
                    iitake3(iipar)=1;
                    tmpdat3{iipar}=mdsdata([DTNE1 ch(iipar,:) echant]);
                end
                if any(find(II-iipar-24==0)-chords==0)
                    iitake4(iipar)=1;
                    tmpdat4{iipar}=mdsdata([DTNE1 ch(iipar,:) echant]);
                end
                if any(find(II-iipar-32==0)-chords==0)
                    iitake5(iipar)=1;
                    tmpdat5{iipar}=mdsdata([DTNE2 ch(iipar,:) echant]);
                end
                if any(find(II-iipar-40==0)-chords==0)
                    iitake6(iipar)=1;
                    tmpdat6{iipar}=mdsdata([DTNE2 ch(iipar,:) echant]);
                end
                if any(find(II-iipar-48==0)-chords==0)
                    iitake7(iipar)=1;
                    tmpdat7{iipar}=mdsdata([DTNE2 ch(iipar,:) echant]);
                end
                if any(find(II-iipar-56==0)-chords==0)
                    iitake8(iipar)=1;
                    tmpdat8{iipar}=mdsdata([DTNE2 ch(iipar,:) echant]);
                end
                mdsclose();
            end
            matlabpool close;
            for iit=find(iitake1)
                tmpdata(:,iit)=tmpdat1{iit};
            end 
            for iit=find(iitake2)
                tmpdata(:,iit+8)=tmpdat2{iit};
            end 
            for iit=find(iitake3)
                tmpdata(:,iit+16)=tmpdat3{iit};
            end 
            for iit=find(iitake4)
                tmpdata(:,iit+24)=tmpdat4{iit};
            end 
            for iit=find(iitake5)
                tmpdata(:,iit+32)=tmpdat5{iit};
            end 
            for iit=find(iitake6)
                tmpdata(:,iit+40)=tmpdat6{iit};
            end 
            for iit=find(iitake7)
                tmpdata(:,iit+48)=tmpdat7{iit};
            end 
            for iit=find(iitake8)
                tmpdata(:,iit+56)=tmpdat8{iit};
            end 
            mdsopen(shot);

        else
        % end of parallel part using parfor
        % beginning of serial part
            ...
            for ii=1:32,
                if any(find(II-ii==0)-chords==0)
                    tmpdata(:,ii)=mdsdata([DTNE1 ch(ii,:) echant]);
                end
                if any(find(II-ii-32==0)-chords==0)
                    tmpdata(:,ii+32)=mdsdata([DTNE2 ch(ii,:) echant]); 
                end	
            end 
            % if data is on the TCV1 server
            if(shot<0)
                mdsclose;
                mdsdisconnect;
                fprintf('- deconnected from TCV1.\n');
                mdsopen(shot);
            end
        % end of serial part that has parallel versions
        end
...
if strcmp(detec,'bot')|strcmp(detec,'both'),
	...
	if load_opt
		...
		%
		for ii=1:32,
			if any(def.top.nchords+find(II-ii==0)-chords==0)
				tmpdata(:,ii)=mdsdata([DTNE3 ch(ii,:) echant]);
			end
		end
		% 
		% Calibrate
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
case 'v' 	
disp('Load the detectors high voltage value')
...
mdsclose;
mdsdisconnect;
disp('Load the detectors geometry')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
case 'f'
disp('Load the MPX filters and detection gas')
if shot<26765,
	...
	if exist('pos')==1&sum(pos==[1 2 3 4])==1,
...
case 3
	if strcmp(detec,'top')|strcmp(detec,'both'), 
		...
		if ~strcmp(abso.material,'nothing'),
			I=0;
			for ii=1:length(mpx.top.filters.material),
				if strcmp(abso.material,mpx.top.filters.material{ii})
					I=ii;
				end
			end
			if I>0,
	...
	if strcmp(detec,'bot')|strcmp(detec,'both'), 
		...
		if ~strcmp(abso.material,'nothing'),
			I=0;
			for ii=1:length(mpx.bot.filters.material),
				if strcmp(abso.material,mpx.bot.filters.material{ii})
					I=ii;
				end
			end
			if I>0,
		...
		if ~isempty(I)&rem(I(end),2)~=0, % Means there was an additional absorber between the 2 detectors
			II=0;
			for ii=1:length(mpx.bot.filters.material),
				if strcmp(addi_material(rem(I(end),2)),mpx.bot.filters.material{ii}),
					II=ii;
				end
			end
			if II>0,
...
case 'e'
if strcmp(detec,'top')|strcmp(detec,'both'),
	disp('Load the top detector efficiency')
	s='';
	for ii=1:length(mpx.top.filters.material)
		s=[s mpx.top.filters.material{ii} num2str(mpx.top.filters.thickness(ii)*1e6) ', '];
	end
	s(end-1:end)=' $';
...
if strcmp(detec,'bot')|strcmp(detec,'both'),
	disp('Load the bottom detector efficiency')
	s='';
	for ii=1:length(mpx.bot.filters.material)
		s=[s mpx.bot.filters.material{ii} num2str(mpx.bot.filters.thickness(ii)*1e6) ', '];
	end
	s(end-1:end)=' $';
...
mdsclose;
mdsdisconnect;
disp('Calculate rho_psi for each detector chord')
if ~exist('opt')
	opt=def.opt;
end;
if exist('psi')~=1,
	if ~exist('rtime'), 
		if exist('time')&~ischar(time)
			rtime=time;
		else
			rtime=def.rtime;
		end;
	end
	if ~exist('liuqe')|isempty(find(repmat(liuqe,1,3)==[1 2 3]))
		liuqe=def.liuqe;
	end;
	mdsopen(shot);
    mdsdata('reset_public()');
