...
def LoadAtomData( file):
    ...
    for i_ion in range(n_ions):
        ...
        ions_data.append(array(plsx)-6)#W/m^3
...
def LoadFile( file):
    ...
    Te_label = f.readline() #remove splitter
    fact = 1e3 if 'keV'in Te_label else 1 
    n_te = int(n_te)
    Te = []
    while len(Te)< n_te:
        line = f.readline()
...
def CalcBackgroundRadiation(ne_data,te_data,Zeff,filt=5,main_ion='D',impurity = 'B',version='puetti_'):
    ...
    Z_i = CalcZ(te_data,element=impurity )
    ...
    ne_data = maximum(ne_data,1)  #troubles with values close to zero
...
def main():
    ...
    te,coeff = LoadFile('lz_puetti_sxr_14.dat')
    te2,coeff2 = LoadFile('lz_puetti_sxr_15.dat')

    Z_W = CalcZ( te,element='W')
    #Z_Si = CalcZ( te,element='Si')
    #Z_C = CalcZ( te,element='C')
    #Z_H = CalcZ( te,element='H')
    #Z_Ca = CalcZ( te,element='Ca')

   
    clf()

    #plot(te, coeff['H']/Z_H**2,label='H')
    #loglog(te, coeff['C']/Z_C**2,'k',label='C')
    #loglog(te, coeff['Si']/Z_Si**2,'b',label='Si')
    #loglog(te, coeff['Ca']/Z_Ca**2,'y',label='Ca')
    loglog(te, coeff['W'] ,'r',label='W/10')

    #loglog(te2, coeff2['C']/Z_C**2,'k--')
    #loglog(te2, coeff2['Si']/Z_Si**2,'b--')
    #loglog(te2, coeff2['Ca']/Z_Ca**2,'y--')
    #loglog(te2, coeff2['W']/Z_W**2/10,'r--')
    


    xlabel('T$_e$ [eV]')
    ylabel('L$_z$/Z$^2$ [Wm$^3$]')
    ylim(1e-37, None)
    xlim(250, 11000)
    grid('on')
