...
class WU:
  ...
  def __init__(self,uf_d,udir='',dev='AUGD'):
    ...
    if 'comm' in uf_d.iterkeys():
      comment=uf_d['comm']
    if udir == '':
      udir=os.getenv('HOME')+'/udb/'+sshot
    os.system('mkdir -p '+udir)
    uf=udir+'/'+prestr+sshot+'.'+extstr
    if os.path.isfile(uf):
      ufbak=uf+'~'
      print 'cp '+uf+' '+ufbak
      shutil.copy2(uf,ufbak)
    print 'Writing file ', uf
...
# Header
    ...
    if ndim > 0:
      ...
      ufs += '0' + ' ' * 29 +';-PROC CODE- 0:RAW 1:AVG 2:SM. 3:AVG+SM\n'
...
# === Headers
    ...
    for j in range(nlines):
      ...
      if l.find("-INDEPENDENT VARIABLE LABEL") !=-1:
        for var in self.vars:
          if l.find(var+'-') != -1:
            self.label[var]=l.split(";-")[0][1:]
            self.nvar +=1
        if self.nvar == 0:
          self.label['X']=l.split(";-")[0][1:]
          self.nvar=1

      for var in self.vars:
        if l.find("-# OF "+var[0]+" PTS-") != -1:
          self.n[var] = int(a[0])
...
# === Data
    if self.dim != self.nvar:
      print " inconsistency in the number of independent variables"
      sys.exit()

    list_var=self.vars[0:self.nvar]
...
# ... F      
    ...
    if self.dim == 1:
      ...
      self.fvalues = ftmp.reshape(self.n['Y'],self.n['X']).transpose()

    elif self.dim == 3:
      self.fvalues = ftmp.reshape(self.n['Z'],self.n['Y'],self.n['X']).transpose()
